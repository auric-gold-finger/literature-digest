"""
PubMed search and fetch utilities using Biopython's Entrez API.
"""
...
def get_entrez_email() -> str:
    """Get NCBI email from Streamlit secrets."""
...
def fetch_pubmed_details(pmids: list[str]) -> list[dict]:
    ...
    Returns:
        List of article dictionaries with title, abstract, date, doi, url, source
    """
    if not pmids:
        return []
...
# Default search topic for longevity research
DEFAULT_TOPIC = (
    '(aging[MeSH] OR "healthy aging"[tiab] OR longevity[tiab] OR healthspan[tiab] OR "biological age"[tiab]) '
    'AND '
    '(rapamycin[tiab] OR metformin[tiab] OR "caloric restriction"[tiab] OR "intermittent fasting"[tiab] OR fasting[tiab] '
    'OR senolytic*[tiab] OR NAD[tiab] OR NMN[tiab] OR "mTOR"[tiab] OR "spermidine"[tiab] '
    'OR "epigenetic clock"[tiab] OR "DNA methylation"[tiab] OR proteomic*[tiab] OR metabolomic*[tiab] OR biomarker*[tiab] '
    'OR "insulin resistance"[tiab] OR hyperinsulinemia[tiab] OR "metabolic syndrome"[tiab] OR "GLP-1"[tiab] '
    'OR "cardiovascular disease"[tiab] OR atherosclerosis[tiab] OR "coronary artery disease"[tiab] '
    'OR dementia[tiab] OR "Alzheimer disease"[tiab] OR neurodegeneration[tiab] '
    'OR exercise[tiab] OR "physical activity"[tiab] OR "resistance training"[tiab] OR HIIT[tiab] OR VO2[tiab])'
)
